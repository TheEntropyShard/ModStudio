...
public class OpenProjectWelcomeButton extends Card {
    private static final int SIDE_SIZE = 64;
    private static final Dimension PREFERRED_SIZE = new Dimension(
        OpenProjectWelcomeButton.SIDE_SIZE, OpenProjectWelcomeButton.SIDE_SIZE
    );

    public OpenProjectWelcomeButton() {
        super(5, 10);

        this.setLayout(new BorderLayout());

        this.add(new JLabel(Icons.get("open_disk").derive(32, 32)), BorderLayout.CENTER);
    }

    @Override
    public Dimension getPreferredSize() {
        return OpenProjectWelcomeButton.PREFERRED_SIZE;
    }
}
